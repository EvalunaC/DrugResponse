...
CV28_alldrug <- function(drug_data,drug){
set.seed(1)
################################
####   Split the data       ####
################################
...
methods_model<- list(model_GR,model_rf,model_pcr,model_pls,model_KNN,model_svm,model_treebag,model_EN,model_ridgeglm,model_Lasso_1)
...
for (i in 1:8){
  cat(i)
  preds<-predict(methods_model[i],testFrame)
  result[[i]] <- eval_result(drug,method[i],unlist(preds))
}

preds <- predict(model_ridgeglm, s = best_lam_0, newx=as.matrix(testFrame[,-1]))
result[[9]] <- eval_result(drug,method[9],unlist(preds))
preds <- predict(model_Lasso_1, s = best_lam_1, newx=as.matrix(testFrame[,-1]))
result[[10]] <- eval_result(drug,method[10],unlist(preds))
...
for (i in 1:length(possibleDrugs2)){
  ...
  CV28_alldrug(drug_data, possibleDrugs2[1])
  }
